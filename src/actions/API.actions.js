    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if (actions.indexOf('websearch') >= 0) {
            ...
            error: function(xhr, status, error) {
              if (xhr.status === 404) {
                receivedMessage.text =
                  'Some error occurred while sending your message!';
              }
              if (status === 'timeout') {
          ...
          let actionJson =
            receivedMessage.response.answers[0].actions[actionIndex];
          let count = -1;
          if (actionJson.hasOwnProperty('count')) {
            count = actionJson.count;
          }
...
export function getTTSLangText(voiceList) {
  ...
  voiceList.forEach((voice, index) => {
    let url =
      'https://translate.googleapis.com/translate_a/single?client=gtx&sl=en-US&tl=' +
      voice.lang +
      '&dt=t&q=' +
      defText;
    $.ajax({
