...
function applyTheme() {
  // body background
  $('.susi-sheet-content-container').css(
    'background-color',
    botbuilderBackgroundBody,
  );
  ...
  $('head').append(
    $(
      '<style>.susi-comment-body-container-user:after { border-color:' +
        `transparent transparent ${botbuilderUserMessageBackground} ${botbuilderUserMessageBackground} !important` +
        '}</style>',
    ),
  );
  $('.susi-comment-by-user .susi-comment-body-container').css(
...
function enableBot() {
  $(document).ready(function() {
    ...
    const mybot =
      ...
      '<div class="susi-sheet-content-container" style="background-color:' +
      botbuilderBackgroundBody +
      ...
      '<polygon points="0 497.25 535.5 267.75 0 38.25 0 216.75 382.5 267.75 0 318.75" fill="#2180c0"/>' +
      '</pre>' +
    ...
    function createSusiMessageAnchor(text, link, msgNumber) {
      $('#susiMsg-' + msgNumber + ' .susi-msg-content-div').html(
